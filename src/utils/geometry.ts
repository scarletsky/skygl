...
import { VertexBuffer } from "graphics";
import { Vec3, Vec2 } from "math";

// from http://www.terathon.com/code/tangent.html
export function calculateTangents(geometry: Geometry) {
    const positions = geometry.attributes[Geometry.ATTRIBUTE_POSITION].data;
    const normals = geometry.attributes[Geometry.ATTRIBUTE_NORMAL].data;
    const uvs = geometry.attributes[Geometry.ATTRIBUTE_TEXCOORD_0].data;
    const indices = geometry.primitive.indexBuffer.data;
    ...
    geometry.addAttribute(Geometry.ATTRIBUTE_TANGENT, new VertexBuffer(tangents, 4));
}
